...
def report(request):
    ...
    if settings.DEBUG == True:
        ...
        myrequest = Request('http://10.1.1.7:8000/data/?format=json') # Live

    response = urlopen(myrequest)
    raw_data = response.read()
    data = json.loads(raw_data)
    data = pd.io.json.json_normalize(data) # Breaks nested JSON to single layer
    ...
    # GET date fields or None for not entered yet
    ...
    # Debugging
    # pdb.set_trace()
    df = df[(df['date'] >= start) & (df['date'] <= end)] # User Input df
    ...
    try:
        ...
        # Sum Numbers
        #df_calc['monthly'] = pd.to_datetime(df_calc['time.date_select']).map(lambda x: str(x.year)+"-"+str(x.month)) ## Get a monthly field
        ...
        request.session['table_display'] = table_display.to_json()
        ...
        if percent=='2':
            ...
            pd.options.display.float_format = '{:}'.format # Change display format
